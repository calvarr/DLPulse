...
# ``flet run`` sets this from pyproject.toml; plain ``python main.py`` / ``runpy`` often does not.
# Without ``full``, the desktop bundle may omit the native Video view → gray area or broken player.
os.environ.setdefault("FLET_DESKTOP_FLAVOR", "full")
...
if _FLET_VIDEO_IMPORT_ERROR is not None:
    ...
    try:
        ...
        if sys.platform.startswith("linux"):
            _player_debug(
                f"libmpv (Linux): {_libmpv or 'NOT FOUND — install mpv / libmpv (e.g. pacman -S mpv) for embedded player'}"
            )
        elif os.name == "nt":
            _player_debug(
                "libmpv: Windows uses media_kit’s bundled native libs (find_library may be empty — this is normal)."
            )
        else:
            _player_debug(f"libmpv find_library: {_libmpv or 'not resolved'}")
...
def _flet_video_configuration_for_platform():
    """Return ``VideoConfiguration`` overrides for the current OS, or ``None`` for defaults.

    On **Windows** and **Linux**, media_kit + Flutter often show a gray/black texture with GPU
    paths while audio works (e.g. https://github.com/media-kit/media-kit/issues/1048,
    Flutter 3.38+ EGL on Linux https://github.com/media-kit/media-kit/issues/1404).

    We default to software-friendly output unless ``DLPULSE_VIDEO_HW_ACCEL=1`` opts into
    the default GPU path. **macOS** keeps media_kit defaults (usually stable).
    """
    if _FLET_VIDEO_IMPORT_ERROR is not None or Video is None:
        return None
    if sys.platform == "darwin":
        return None
    if not (os.name == "nt" or sys.platform.startswith("linux")):
        return None
    if (os.environ.get("DLPULSE_VIDEO_HW_ACCEL", "") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
        _player_debug("DLPULSE_VIDEO_HW_ACCEL=1 — using default media_kit video acceleration.")
        return None
    try:
        from flet_video.types import VideoConfiguration
    except Exception as _e:
        _player_debug(f"VideoConfiguration import failed: {_e}")
        return None
    _player_debug(
        f"{sys.platform}: embedded player uses software-friendly VideoConfiguration "
        "(enable_hardware_acceleration=False, hwdec=no). "
        "Set DLPULSE_VIDEO_HW_ACCEL=1 to try the GPU-accelerated path."
    )
    return VideoConfiguration(
        enable_hardware_acceleration=False,
        mpv_properties={"hwdec": "no"},
    )
...
def _write_temp_m3u_playlist(paths: list[Path]) -> Path:
    ...
    return _write_temp_m3u_entries([p.as_uri() for p in resolved], titles=[p.name for p in resolved])


def _write_temp_m3u_entries(entries: list[str], *, titles: list[str] | None = None) -> Path:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if titles and i < len(titles):
                label = str(titles[i] or "").strip().replace("\n", " ")
            if not label:
                label = f"Track {i + 1}"
            label = label[:240]
...
def play_media_files(paths: list[Path]) -> None:
    ...
    if not argv:
        raise ValueError("Player command is empty.")
    argv = _inject_mpv_playlist_prefetch(argv, pl_str)
...
def _inject_mpv_playlist_prefetch(argv: list[str], target: str) -> list[str]:
    """Help mpv advance through multi-URL ``.m3u`` playlists (network streams)."""
    if not target.lower().endswith(".m3u"):
        return argv
    if not argv:
        return argv
    if Path(argv[0]).name.lower() not in ("mpv", "mpv.exe"):
        return argv
    joined = " ".join(argv).lower()
    if "prefetch-playlist" in joined:
        return argv
    return [argv[0], "--prefetch-playlist=yes", *argv[1:]]


def _inject_vlc_playlist_network(argv: list[str], target: str) -> list[str]:
    """VLC is picky about rapid localhost playlist advances; add a modest network cache."""
    if not target.lower().endswith(".m3u"):
        return argv
    if not argv:
        return argv
    if Path(argv[0]).name.lower() not in ("vlc", "vlc.exe"):
        return argv
    joined = " ".join(argv).lower()
    if "network-caching" in joined:
        return argv
    return [argv[0], "--network-caching=5000", *argv[1:]]


def play_stream_urls(urls: list[str], entry_titles: list[str] | None = None) -> None:
    """
    Play page URLs (YouTube, SoundCloud, …) without downloading, via the local relay
    ``http://127.0.0.1:<port>/remote_stream?u=…``. yt-dlp resolves the media URL; a single progressive
    stream is served via HTTP proxy only (no ffmpeg). ffmpeg is used only for DASH (separate video/audio).
    The external player receives one stream or a temporary ``.m3u`` playlist.

    ``entry_titles`` (optional, same indices as ``urls``) labels each track in the temp M3U.
    """
    clean: list[str] = []
    m3u_labels: list[str] = []
    for i, u in enumerate(urls):
        ...
        label = ""
        if entry_titles and i < len(entry_titles):
            label = str(entry_titles[i] or "").strip().replace("\n", " ")
        if not label:
            label = f"Stream {len(clean)}"
        m3u_labels.append(label[:240])
    if not clean:
    ...
    target = relay[0] if len(relay) == 1 else str(_write_temp_m3u_entries(relay, titles=m3u_labels))
    argv = _inject_mpv_playlist_prefetch(argv, str(target))
    argv = _inject_vlc_playlist_network(argv, str(target))
...
def _library_row_subtitle(st: os.stat_result) -> str:
...
def _run_minimal_player_mode(page: ft.Page) -> bool:
    ...
    _cfg = _flet_video_configuration_for_platform()
    if _cfg is not None:
        _vkw["configuration"] = _cfg
    video = Video(**_vkw)
    playing = {"value": False}
...
def main(page: ft.Page) -> None:
    ...
    def use_browser_player() -> bool:
        return get_playback_mode() == "browser"

    async def _browser_open_watch_query(query: str) -> None:
        """Open ``/watch?…`` in the default browser (``query`` without leading ``?``)."""
        if not is_cast_server_running():
            await asyncio.to_thread(start_cast_server, "127.0.0.1", 0)
        port = get_cast_server_port()
        if port <= 0:
            raise RuntimeError("Local HTTP server did not start (needed for browser playback).")
        url = f"http://127.0.0.1:{port}/watch?{query}"
        await asyncio.to_thread(webbrowser.open, url)

    async def play_browser_library_async(items: list[tuple[str, Path]]) -> None:
        if not items:
            return
        if not is_cast_server_running():
            await asyncio.to_thread(start_cast_server, "127.0.0.1", 0)
        port = get_cast_server_port()
        if port <= 0:
            set_status("Browser playback: could not start local HTTP server.")
            page.update()
            return
        try:
        ...
        path0 = items[0][1].expanduser().resolve()
        try:
            rel = path0.relative_to(base).as_posix()
            q = f"p={quote(rel, safe='')}"
        except ValueError:
            try:
                reg = register_media_path(path0)
            except OSError as e:
                set_status(f"Browser playback: {e}")
                page.update()
                return
            q = f"p={quote(reg, safe='')}"
        try:
            await _browser_open_watch_query(q)
        except Exception as e:
            set_status(f"Browser playback: {e}")
            page.update()
            return
        msg = f"Opened in browser: {path0.name}"
        if len(items) > 1:
            msg += f" (first of {len(items)} selected)."
        set_status(msg)
        _player_debug(f"browser library: {path0}")
        page.update()

    async def play_browser_search_streams_async(urls: list[str]) -> None:
        ...
        try:
            async with async_busy(
                "Search & Download — extracting stream for browser…",
                min_display_s=0.5,
            ):
                if not is_cast_server_running():
                    await asyncio.to_thread(start_cast_server, "127.0.0.1", 0)
                port = get_cast_server_port()
                if port <= 0:
                    set_status("Browser playback: HTTP server did not start.")
                    page.update()
                    return

                async def _resolve_one(url: str) -> str | None:
                    pair = await asyncio.to_thread(extract_split_video_audio_stream_urls, url)
                    if pair:
                        v, a = pair
                        return (
                    ...
                    direct = await asyncio.to_thread(extract_single_http_stream_url, url)
                    return direct

                results = await asyncio.gather(*(_resolve_one(u) for u in cleaned))
            first = next((r for r in results if r), None)
            if not first:
                set_status("Could not extract a stream URL for browser playback.")
                page.update()
                return
            loc = f"http://127.0.0.1:{port}"
            if first.startswith(loc) and "/direct_stream?" in first:
                pu = urlparse(first)
                qd = parse_qs(pu.query)
                v0 = (qd.get("v") or [""])[0]
                a0 = (qd.get("a") or [""])[0]
                if not v0:
                    set_status("Browser playback: empty direct_stream URL.")
                    page.update()
                    return
                qs = f"dv={quote(v0, safe='')}"
                if a0:
                    qs += f"&da={quote(a0, safe='')}"
                await _browser_open_watch_query(qs)
                set_status("Stream opened in your default browser (HTML5 player).")
                _player_debug(f"browser stream: direct_stream v_len={len(v0)} a_len={len(a0)}")
            elif first.startswith(loc) and "/remote_stream?" in first:
                pu = urlparse(first)
                qd = parse_qs(pu.query)
                u0 = (qd.get("u") or [""])[0]
                if not u0:
                    set_status("Browser playback: empty relay URL.")
                    page.update()
                    return
                await _browser_open_watch_query(f"relay=1&u={quote(u0, safe='')}")
                set_status("Stream opened in your default browser.")
                _player_debug("browser stream: remote_stream relay")
            else:
                set_status(
                    "Browser mode needs a local relay URL (not a raw HTTPS stream). "
                    "Use Internal or External player, or download the file first."
                )
                _player_debug(f"browser stream: unsupported resolved URL: {first[:100]!r}")
        except Exception as e:
            msg = str(e) or type(e).__name__
            _player_debug(f"browser stream play error: {type(e).__name__}: {msg}")
            set_status(f"Browser stream error: {msg}")
        page.update()
    ...
    def _result_row_display_title(item: dict) -> str:
    ...
    def _selected_result_urls() -> list[str]:
        return [(e.get("url") or "").strip() for e in _selected_result_entries()]
    ...
    def rebuild_results() -> None:
        ...
        for item in items:
            ...
            title_line = _result_row_display_title(item)
            title_disp = title_line[:120]
                ...
                async def _row_play(
                    ...
                    try:
                        async with async_busy(
                            ...
                            await asyncio.to_thread(play_stream_urls, [url], [row_title])
    ...
    async def on_play_selected(_: ft.ControlEvent) -> None:
        if st.active_result_kind not in ("search", "playlist"):
            ...
            page.update()
            return
        entries = _selected_result_entries()
        urls = [(e.get("url") or "").strip() for e in entries]
        titles = [_result_row_display_title(e) for e in entries]
        if not urls:
        ...
        if use_internal_player():
            ...
            return
        if use_browser_player():
            await play_browser_search_streams_async(urls)
            return
        try:
            async with async_busy(
                ...
                await asyncio.to_thread(play_stream_urls, urls, titles)
    ...
    def refresh_library() -> None:
        ...
        if not st.lib_rows:
            ...
            for idx, (rel, p) in enumerate(st.lib_rows[:400]):
                ...
                try:
                    sub = _library_row_subtitle(p.stat())
                except OSError:
                    sub = "—"
                lib_list.controls.append(
                    ft.ListTile(
                        ...
                        subtitle=ft.Text(sub, size=11, color=ft.Colors.GREY_500),
                    )
                )
        update_lib_sel_hint()
    ...
    async def on_play(_: ft.ControlEvent) -> None:
        ...
        if use_internal_player():
            ...
            return
        if use_browser_player():
            await play_browser_library_async(items)
            return
        try:
    ...
    player_hint = ft.Text(
        ...
        "If the built-in (flet-video) player shows a gray area: Settings → “Browser HTML5” or “External player”. "
        "Modul browser deschide Chrome/Edge/Firefox; nu folosește media_kit.",
        size=11,
        color=ft.Colors.GREY_500,
    )
    ...
    def _make_player_video(playlist: list | None = None, *, autoplay: bool = False) -> ft.Control:
        ...
        # Windows + Linux: ``_flet_video_configuration_for_platform()`` prefers a software-friendly
        # path (gray/black texture with GPU; media-kit#1048, Linux EGL Flutter 3.38+ #1404).
        # macOS: leave defaults. Opt-in GPU: DLPULSE_VIDEO_HW_ACCEL=1.
        common: dict = dict(
        ...
        _cfg = _flet_video_configuration_for_platform()
        if _cfg is not None:
            common["configuration"] = _cfg
        # flet-video ≥ 0.85: use MaterialDesktopVideoControls (show_controls is deprecated).
        try:
            from flet_video import MaterialDesktopVideoControls
            ...
            _player_debug(f"MaterialDesktopVideoControls / Video build: {type(ex).__name__}: {ex}")
    ...
    def _make_browser_player_placeholder() -> ft.Control:
        return _make_player_placeholder(
            "Browser playback (HTML5): use Play in Library or Search — your default browser opens a local page. "
            "Codecs depend on the browser; no flet-video / media_kit."
        )
    ...
    async def on_player_play_pause(_: ft.ControlEvent) -> None:
        if Video is None or not st.player_items:
            if use_browser_player():
                set_status("Browser mode: use Play in Library or Search (this tab does not host the video).")
            else:
    ...
    async def on_player_fullscreen(_: ft.ControlEvent) -> None:
        ...
        video = getattr(st, "player_video", None)
        if video is None or not st.player_ready:
            set_status("Player is loading. Try again.")
            page.update()
            return
        try:
    ...
    player_surface = ft.Container(
        content=(
            ...
            if use_internal_player()
                ...
                _make_browser_player_placeholder()
                if use_browser_player()
    ...
    win_nt_video_hint = ft.Text(
        "Windows: the built-in (flet-video) player may play audio only (no picture) on some GPUs (driver / ANGLE). "
        "Deschide Settings → „Browser HTML5” sau „External player”, apoi Save.",
        size=11,
        color=ft.Colors.AMBER_200,
        visible=os.name == "nt",
    )
    ...
    tab_player = ft.Column(
        [
            ft.Row(
                [
                    ft.Column(
                        [player_title, player_hint, win_nt_video_hint],
                        spacing=3,
                        tight=True,
                        expand=True,
                    ),
    ...
    dd_playback_mode = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key="external",
                text="External player (mpv / VLC / OS default)",
            ),
            ft.dropdown.Option(
                key="internal",
                text="Internal app player (in development · flet-video / media_kit)",
            ),
            ft.dropdown.Option(
                key="browser",
                text="Browser HTML5 (default browser, no media_kit)",
            ),
        ],
    )
    ...
    _settings_scroll_body = ft.Column(
        [
            ...
            ft.Container(
                ...
                content=ft.Row(
                    [
                        ...
                        ft.Text(
                            "Default on first install: external player. "
                            "The in-app player is still in development (video may be missing). "
                            "For multiple Search results with an external player, mpv usually handles playlists better than VLC.",
                            size=12,
                            color=ft.Colors.GREY_200,
                            expand=True,
                        ),
            ...
            ft.Text(
                "Choose what the Play button uses: external player (recommended), in-app player (experimental), or browser HTML5. "
                "On first run with no saved settings, playback uses the external player.",
                size=11,
                color=ft.Colors.GREY_500,
            ),
            dd_playback_mode,
            ft.Text(
                "The in-app (flet-video) player is still in development: on many systems the video area stays gray or is audio-only. "
                "For Search streams the app starts a local relay; ffmpeg is used only when YouTube "
                "serves separate video and audio URLs (DASH)—not for a single progressive stream.",
                size=11,
                color=ft.Colors.GREY_400,
            ),
            ft.Text(
